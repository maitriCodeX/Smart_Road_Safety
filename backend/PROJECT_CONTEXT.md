...
## 7. Android App Frontend Contract API Reference

All 6 modules requested by the Android Studio frontend team are live, tested, and 100% compliant:

| Module | Method | Endpoint | Description / Sample Request |
| :--- | :--- | :--- | :--- |
| **Auth Register** | `POST` | `/api/auth/register` | Body: `{"name", "email", "password", "confirm_password"}`. Returns JWT token & user. |
| **Auth Login** | `POST` | `/api/auth/login` | Body: `{"email", "password"}`. Returns JWT token & user profile. |
| **User Profile** | `GET` | `/api/user/profile` | Returns full profile, blood group, car plate, and `relative_phone_number` for SOS. |
| **Update Profile** | `POST` / `PUT` | `/api/user/profile` | Updates profile, picture, and syncs emergency relative contact to Dead Man's Switch. |
| **Start Ride** | `POST` | `/api/ride/start` | Body: `{"destination", "current_lat", "current_lon"}`. Returns distance, time & real-time route alerts. |
| **Active Nav Alerts** | `GET` | `/api/navigation/alerts` | Returns active route alerts (Potholes, Blackspots, Speed limits, Blind curves). |
| **Frequent Routes** | `GET` | `/api/hazards/frequent-routes` | Returns list of frequent routes with hazard counts (Gandhinagar-Mehsana, SG Highway, etc.). |
| **Route Hazards Detail** | `GET` | `/api/hazards/route-detail?route_id=ROUTE_GNR_MEH_01` | Returns individual route hazards with coordinates, severity, and descriptions. |
| **Driver Score & Tickets** | `GET` | `/api/driver/score` | Returns overall score, actuarial tier, 4 pillars (`smoothness_pct`, `cornering_pct`, etc.), and `fine_tickets`. |
| **Hazard Heatmap** | `GET` | `/api/heatmap` | Returns 132 Gaussian weighted points and blackspot zones with historical crash stats. |
